...
<thead>
	<th colspan="2">No. Pemeriksaan <?php
		$cekpemeriksaan = mysql_query("select * from resep where no_resep='$getnrs'");
		$cek = mysql_fetch_array($cekpemeriksaan);
		...
		<?php }else{?>
				<form method="post" action="">
					<div class="col-sm-8">
						<select name="no_pemeriksaan" class="form-control"><?php
			              $qpl=mysql_query("SELECT * FROM pemeriksaan ORDER BY no_pemeriksaan DESC"); 
			              while($pl=mysql_fetch_array($qpl)){
				...
				if(isset($_POST['cekpem'])){
					...
					$query = mysql_query("update resep set no_pemeriksaan='$no_pemeriksaan' where
						no_resep='$getnrs'");
					if($query == 1){
						echo "<script>document.location.href='".$urlsaatini."'</script>";
					}else{
						echo "GAGAL EDIT NO PEMERIKSAAN DI RESEP".mysql_error();
					}
				}
				?>
		<?php }  ?>
	...
	<th colspan="3">
		...
		<div class="col-sm-8">
			<select name='cari' class='form-control'><?php
	            $qpl=mysql_query("SELECT * FROM obat ORDER BY nama_obat ASC"); 
	         while($pl=mysql_fetch_array($qpl)){
	            if ($pl['kode_obat']){
...
if(isset($_POST['scari'])){
	$cr = $_POST['cari'];
	$qt = mysql_query("select * from obat where kode_obat like '%$cr%'");
	$t = mysql_fetch_array($qt); 
}

$query = mysql_query("select * from detail_resep where no_resep='$getnrs'");
$no = 1;
while($a = mysql_fetch_array($query)){ ?>

<tr>
	<td><?php echo $no; ?></td>
	<td><?php $qprod = mysql_query("select * from obat where kode_obat='$a[kode_obat]'");
		while($prod = mysql_fetch_array($qprod)){
...
if($_GET['del'] == 'HapusObat'){
	...
	$qh = mysql_query("delete from detail_resep where kode_obat='$gtdelobt' and no_resep='$getnrs'");
	if($qh == 1){
...
if(isset($_POST['tmbobat'])){
	...
	$query = mysql_query("insert into detail_resep values('$getnrs','$kode_obat','$dosis','$jml_obat','$total_awal')");
	if($query == 1){
		echo "<script>document.location.href='".$urlsaatini."';</script>";
	}else{
		echo "GAGAL Tambah Obat".mysql_error();
	}
}
...
<tr>
<?php $qh = mysql_query("select total_awal, sum(total_awal) as jumlah from detail_resep where 
	no_resep='$getnrs'");
$c = mysql_fetch_array($qh); ?>
	...
	if(isset($_POST['akhiri'])){
		$hrgakhir = $c['jumlah'];
		$query = mysql_query("update resep set harga_akhir='$hrgakhir' where no_resep='$getnrs'");
		if($query == 1){ ?>
			<script type="text/javascript">
		...
		<?php }else{
			echo "GAGAL SIMPAN DATA AKHIR RESEP".mysql_error();
		}
	}
	?>
	</td>
