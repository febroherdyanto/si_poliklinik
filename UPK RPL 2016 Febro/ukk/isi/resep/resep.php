...
if(isset($_POST['act'])){
	$no_resep = '';
	$query = mysql_query("insert into resep (no_resep) values('$no_resep')");
	if($query == 1){
		$qs = mysql_query("select * from resep order by no_resep desc limit 1");
		while($s = mysql_fetch_array($qs)){
			$nor = $s['no_resep'];
			echo "<script>document.location.href='index.php?halaman=Tambah-Resep&nrs=".$nor."'</script>";
		}
	}else{
		echo "GAGAL INSERT NO RESEP ".mysql_error();
	}
}
?>
-->
...
<div class="row">
	<table id="resep" class="table table-bordered table-hover">
		...
		<tbody>
	<?php $qtampil = mysql_query("select * from resep order by no_resep asc");
		$no = 1;
