...
<?php $getno = $_GET['no'];
$qtampil = mysql_query("select * from pemeriksaan where no_pemeriksaan='$getno'");
while($tampil = mysql_fetch_array($qtampil)){
?>
		...
		<div class="col-md-6">
          ...
          <span class='field'>
            <select name='no_pendaftaran' class='form-control'><?php
              $qpl=mysql_query("SELECT * FROM pendaftaran ORDER BY tgl_pendaftaran DESC"); 
...
$query = mysql_query("update pemeriksaan set no_pendaftaran='$no_pendaftaran', keluhan='$keluhan', diagnosa='$diagnosa', perawatan='$perawatan', tindakan='$tindakan', berat_badan='$berat_badan', tensi_diastolik='$tensi_diastolik', tensi_sistolik='$tensi_sistolik' where no_pemeriksaan='$getno'");
if($query == 1){
