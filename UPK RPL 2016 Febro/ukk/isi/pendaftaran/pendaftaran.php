...
<div class="row">
	<table id="pendaftaran" class="table table-bordered table-hover">
		...
		<tbody>
	<?php $qtampil = mysql_query("select * from pendaftaran order by tgl_pendaftaran DESC");
		$no = 1;
		while($tm = mysql_fetch_array($qtampil)){?>
		<tr>
			<td align="center"><?php echo $no; ?>.</td>
			<td><?php $qnpeg = mysql_query("select * from pegawai where NIP='$tm[NIP]'");
			 while($npg = mysql_fetch_array($qnpeg)){ echo $npg['nama_pegawai']; } ?></td>
			<td><?php $qnpas = mysql_query("select * from pasien where no_pasien='$tm[no_pasien]'");
			 while($nps = mysql_fetch_array($qnpas)){ echo $nps['nama_pasien']; } ?></td>
		...
		<div class="col-md-8">
          ...
          <span class='field'>
            <select name='no_pasien' class='form-control'><?php
              $qpas=mysql_query("SELECT * FROM pasien ORDER BY no_pasien DESC"); 
