...
<div class="row">
	<table id="jadwalpraktek" class="table table-bordered table-hover">
		...
		<tbody>
	<?php $qtampil = mysql_query("select * from jadwal_praktek order by kode_jadwal asc");
		$no = 1;
		while($tm = mysql_fetch_array($qtampil)){?>
		<tr>
			<td align="center"><?php echo $no; ?>.</td>
			<td><?php echo $tm['kode_jadwal'] ;?></td>
			<td><?php $qnama = mysql_query("select * from dokter where 
		...
		<div class="col-md-8">
          <span class='field'>
            <select name='kode_dokter' class='form-control'><?php
              $qdok=mysql_query("SELECT * FROM dokter ORDER BY kode_dokter ASC"); 
              while($dok=mysql_fetch_array($qdok)){
               if ($dok['kode_dokter']){
