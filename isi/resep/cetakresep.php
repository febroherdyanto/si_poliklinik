...
<!-- =================== BIODATA PASIEN ======================= -->
<?php
$q1 = mysql_query("select * from resep where no_resep='$getnrs'");
while($t1 = mysql_fetch_array($q1)){
$no_periksa1 = $t1['no_pemeriksaan'];
	
	//Query Pemeriksaan
	$q2 = mysql_query("select * from pemeriksaan where no_pemeriksaan='$no_periksa1'");
	while($t2 = mysql_fetch_array($q2)){
		$no_daftar1 = $t2['no_pendaftaran'];

		//Query Pendaftaran
		$q3 = mysql_query("select * from pendaftaran where no_pendaftaran='$no_daftar1'");
		while($t3 = mysql_fetch_array($q3)){
			$no_pasien1 = $t3['no_pasien'];
			$nip = $t3['NIP'] ;

			//Query Pegawai
			$qpeg = mysql_query("select * from pegawai where NIP='$nip'");
			$peg=mysql_fetch_array($qpeg);
			$nama_pegawai = $peg['nama_pegawai'];

			//Query Pasien
			$q4 = mysql_query("select * from pasien where no_pasien='$no_pasien1'");
			while($t4 = mysql_fetch_array($q4)){
...
<?php 
$queryresep = mysql_query("select * from detail_resep where no_resep='$getnrs'");
$no = 1;
while($resep = mysql_fetch_array($queryresep)){
	$kode_obat = $resep['kode_obat'];	?>
<tr>
	<td width="2%"><br></td>
	<td><?php echo $no; ?></td>
	<td><?php $queryobat = mysql_query("select * from obat where kode_obat='$kode_obat'");
		$obat = mysql_fetch_array($queryobat); echo $obat['nama_obat']; ?></td>
...
<tr>
	<td colspan="5"><br></td>
	<td><b>Rp. </b></td>
	<td align="right"><b><?php $qtotal = mysql_query("select * from resep where no_resep='$getnrs'");
	while($tot = mysql_fetch_array($qtotal)){ echo number_format($tot['harga_akhir'], 2, ',', '.'); } ?></b></td>
</tr>
