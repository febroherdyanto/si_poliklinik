...
<div class="row">
	<table id="dokter" class="table table-bordered table-hover">
		...
		<tbody>
	<?php $qtampil = mysql_query("select * from dokter order by kode_dokter asc");
		$no = 1;
		while($tm = mysql_fetch_array($qtampil)){?>
		<tr>
			<td align="center"><?php echo $no; ?>.</td>
			<td><?php echo $tm['kode_dokter'] ;?></td>
			<td><?php $qnama = mysql_query("select * from poliklinik where 
				kode_poliklinik='$tm[kode_poliklinik]'");
				while($nama = mysql_fetch_array($qnama)){
		...
		<div class="col-md-8">
          ...
          <span class='field'>
            <select name='kode_pl' class='form-control'><?php
              $qpl=mysql_query("SELECT * FROM poliklinik ORDER BY kode_poliklinik ASC"); 
